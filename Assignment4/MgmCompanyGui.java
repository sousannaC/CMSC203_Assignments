...
public class MgmCompanyGui extends Application {
	...
	private static final int NUM_PIX = 10;
		...
		if (!isMgmFieldEmpty()) {
			if (isTaxFeeInvalid()) {
				...
				if (true) {
					...
					mgmCompany = new MgmCompany(mgmNameText.getText(), mgmIdText.getText(),
	    ...
	    if (!isPropertyFieldEmpty()) {
	        ...
	        switch (rtnValue) {
	            ...
	            default: // the case where property is valid and created - now display icon
	                ...
	                int propWidth = Integer.parseInt(propPlotWidthText.getText());
	                int propDepth = Integer.parseInt(propPlotDepthText.getText());
	                ...
	                icon.setFitWidth(property.getPlot().getWidth() * SCALE_FACTOR);
	                icon.setFitHeight(property.getPlot().getDepth() * SCALE_FACTOR);
	                icon.setSmooth(true);
	...
	public ArrayList<String> shufflePix() {
	    ...
	    for (int i = 0; i < NUM_PIX; i++) {
	        rtnList.add("file:src/images/" + pix[i]); // Use forward slash here
	    }
